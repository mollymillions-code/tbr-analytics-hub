      ...
      <form onSubmit={handleSubmit} className="mb-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-32 py-4 bg-[var(--bg-card)] border-2 border-[var(--border-color)] rounded-xl text-white text-sm font-body placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--accent-cyan)] transition-colors"
          />
          <button
            type="submit"
            disabled={isAnalyzing || !query.trim()}
            className="absolute right-2 top-1/2 -translate-y-1/2 px-5 py-2 bg-[var(--accent-cyan)] text-black font-display text-xs font-bold tracking-wider rounded-lg hover:bg-[#00b8d9] transition-colors disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed flex items-center gap-2"
          >
          ...
          <div className="flex flex-wrap gap-2">
            {SUGGESTED_QUERIES.map((sq) => (
              <button
                ...
                className="px-3 py-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg text-xs text-[var(--text-secondary)] hover:border-[var(--accent-cyan)] hover:text-[var(--accent-cyan)] transition-colors cursor-pointer flex items-center gap-1.5"
              >
      ...
      {result && !isAnalyzing && (
        <div className="space-y-5">
          ...
          {result.blocks.map((block, i) => (
            <div key={i}>
              {block.type === "heading" && (
                <h3 className="font-display text-sm font-bold tracking-wider text-white uppercase mt-4">{block.content}</h3>
              )}
